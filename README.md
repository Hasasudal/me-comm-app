...
- 통합 게시판, 학과 뉴스, 동아리, 공모전 모집의 개별 주소
- 게시글 비밀번호를 이용한 본문·수정·삭제 보호
- 뉴스 승인 대기, 관리자 검토·수정·승인과 수정 후 재승인
...
새 로컬 데이터베이스에는 `drizzle/0000`부터 `drizzle/0003_school_members.sql`까지 순서대로 적용합니다. `.dev.vars.example`을 복사해 `.dev.vars`를 만들고 관리자 코드 해시·salt와 Firebase 공개 설정 네 값을 입력합니다.
