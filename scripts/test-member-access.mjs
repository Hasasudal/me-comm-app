...
try {
  ...
  assert.equal(session.data.email, 'active@ks.ac.kr');
  ...
  assert.equal(
    (await request('/api/posts', { cookie: fixture.activeCookie })).status,
    401,
...
console.log(
  'PASS: anonymous, forged, expired, suspended and logged-out sessions are denied while active members can read.',
);
