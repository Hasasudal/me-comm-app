...
const contest = await request('/api/posts', {
  method: 'POST',
  body: {
    title: `모집 검증 ${suffix}`,
    content: '함께 만들어요',
...
try {
  ...
  assert.equal(detail.data.post.author_name, '상세작성자');
  assert.equal(detail.data.post.prefix, '정보');
  ...
  const edited = await request(`/api/posts/${contest.data.id}`, {
    method: 'PATCH',
    body: {
      title: '모집 수정',
      content: '수정한 모집',
