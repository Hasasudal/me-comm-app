...
type TokenClaims={aud?:unknown;iss?:unknown;sub?:unknown;iat?:unknown;exp?:unknown;email?:unknown;email_verified?:unknown;name?:unknown};

export type FirebaseIdentity={userId:string;email:string;displayName:string};

export class FirebaseTokenError extends Error{
 constructor(message='로그인 인증을 확인해주세요.'){super(message);this.name='FirebaseTokenError';}
}
...
function decodeBase64Url(value:string){
 ...
 let binary:string;try{binary=atob(base64);}catch{throw new FirebaseTokenError();}
 return Uint8Array.from(binary,char=>char.charCodeAt(0));
}

function decodeJson<T>(value:string):T{
 try{return JSON.parse(new TextDecoder().decode(decodeBase64Url(value))) as T;}catch{throw new FirebaseTokenError();}
}

function cacheSeconds(header:string|null){
 const match=header?.match(/(?:^|,)\s*max-age=(\d+)/i);return match?Math.max(60,Number(match[1])):300;
}

async function fetchFirebaseKeys({force=false}:{force?:boolean}={}):Promise<FirebaseKeySet>{
 const now=Date.now();if(!force&&cachedKeys&&cachedKeys.expiresAt>now)return cachedKeys.value;
 ...
 if(!Array.isArray(value.keys)||!value.keys.length)throw new FirebaseTokenError('로그인 인증 서버 응답을 확인할 수 없습니다.');
 cachedKeys={value,expiresAt:now+cacheSeconds(response.headers.get('cache-control'))*1000};return value;
}

async function findKey(kid:string,provider:KeyProvider){
 let set=await provider();let key=set.keys.find(item=>item.kid===kid);
 if(!key){set=await provider({force:true});key=set.keys.find(item=>item.kid===kid);}
...
export async function verifyFirebaseIdToken(token:string,options:VerifyOptions):Promise<FirebaseIdentity>{
 const parts=token.split('.');if(parts.length!==3||!parts.every(Boolean))throw new FirebaseTokenError();
 const header=decodeJson<TokenHeader>(parts[0]);const claims=decodeJson<TokenClaims>(parts[1]);
 ...
 let key:CryptoKey;try{key=await crypto.subtle.importKey('jwk',jwk,{name:'RSASSA-PKCS1-v1_5',hash:'SHA-256'},false,['verify']);}catch{throw new FirebaseTokenError();}
 ...
 if(claims.aud!==options.projectId||claims.iss!==`https://securetoken.google.com/${options.projectId}`)throw new FirebaseTokenError();
