...
test('overlapping marks split into flat segments with memo numbers at mark ends',()=>{
 ...
 assert.deepEqual(result.map(s=>s.memoStarts),[[],[1],[],[2],[],[]]);
...
test('overlap detection treats ranges as half-open',()=>{
 ...
 assert.equal(overlaps(mark,0,2),false);assert.equal(overlaps(mark,4,9),true);assert.equal(overlaps(mark,5,6),false);
});
