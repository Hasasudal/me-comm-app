...
type Identity = { signedIn: boolean; admin?: boolean; displayName?: string; email?: string };
...
const paths: Record<Category, string> = { board: '/board', news: '/news', clubs: '/clubs', contests: '/contests' };
const tabs = [
  { id: 'all', label: '통합 게시판', href: '/', icon: Layers3 },
  { id: 'board', label: '자유게시판', href: '/board', icon: MessageSquare },
  { id: 'news', label: '학과 뉴스', href: '/news', icon: Newspaper },
  { id: 'clubs', label: '동아리', href: '/clubs', icon: Users },
  { id: 'contests', label: '공모전 모집', href: '/contests', icon: Trophy },
] as const;
...
export default function PostDetail({ id }: { id: string }) {
  ...
  async function remove(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      window.location.assign(paths[post.category]);
    } catch (e) {
      setError((e as Error).message);
      setBusy(false);
    }
    ...
    <div className="app-shell">
      <aside className={`sidebar ${mobileNav ? 'mobile-open' : ''}`}>
        ...
        <nav aria-label="주요 메뉴">
          {tabs.map((t) => (
            <a key={t.id} className="nav-item" href={t.href}>
              <t.icon size={20} />
              {t.label}
            </a>
          ))}
      ...
      <div className="main-wrap">
        <header className="topbar">
          <div>
            ...
            <span className="breadcrumb">
              ...
              {category && (
                <>
                  <a href={paths[category]}>{labels[category]}</a>
                  <ChevronRight size={14} />
                </>
              )}
              <b>상세</b>
        ...
        <main className="detail-main">
          <a className="back-link" href={category ? paths[category] : '/'}>
            <ArrowLeft size={17} />
            {news ? '내 기사 목록' : category ? `${labels[category]} 목록` : '게시판 목록'}
          </a>
          {loading ? (
            ...
            post && (
              <section className="detail-card">
                <div className="detail-head">
                  <div>
                    <span className={`category-tag ${post.category}`}>{labels[post.category]}</span>
