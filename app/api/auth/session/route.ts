...
import { expiredSessionCookie, issueMemberSession, revokeCurrentSession, sessionCookie } from '../../../../lib/member-auth';
...
export async function POST(request:Request){return handle(async()=>{
 ...
 const identity=await verifyFirebaseIdToken(idToken,{projectId});const session=await issueMemberSession(identity);
...
export async function DELETE(request:Request){return handle(async()=>{
 await revokeCurrentSession(request);return json({ok:true},200,{'Set-Cookie':expiredSessionCookie()});
});}
