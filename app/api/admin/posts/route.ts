...
export async function GET(request: Request) {
  return handle(async () => {
    ...
    const rows = await db()
      .prepare(
        "SELECT id,title,category,content,prefix,author_name,status,feedback,created_at,updated_at FROM posts WHERE category='news' AND status=? ORDER BY updated_at DESC",
      )
      .bind(status)
      .all<{ feedback: string | null }>();
