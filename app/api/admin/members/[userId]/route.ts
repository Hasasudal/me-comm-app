...
export async function DELETE(request:Request,context:{params:Promise<{userId:string}>}){return handle(async()=>{
 const current=await requireAdmin(request);await input(request);const {userId}=await context.params;
 if(userId===current.userId)throw new HttpError(400,'자신의 관리자 권한은 회수할 수 없습니다.');
 const active=await db().prepare('SELECT COUNT(*) AS count FROM admin_users WHERE revoked_at IS NULL').first<{count:number}>();
